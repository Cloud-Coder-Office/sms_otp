		...
		if ($_SERVER['REQUEST_METHOD'] == "POST") {
		    ...
		    if (!empty($user_id) || !empty($route_id)) {
				...
				for ($i = 0; $i < sizeof($route_id); $i++) {
					...
					if($route_ids==4){
						$sql = "INSERT INTO user_route_api (user_id, route_id,capacity,status) VALUES ('$user_id',4,'$route_capacity[3]','1')";
						
					} 
					if($route_ids==5){
						$sql = "INSERT INTO user_route_api (user_id, route_id,capacity,status) VALUES ('$user_id',5,'$route_capacity[4]','1')";
						
					} 
					if($route_ids==6){
						$sql = "INSERT INTO user_route_api (user_id, route_id,capacity,status) VALUES ('$user_id',6,'$route_capacity[5]','1')";
