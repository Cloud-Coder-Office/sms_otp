...
<?php 
 if(isset($_GET['submit']))
 {
	...
	$yourString   = urlencode($_GET['msg']); 
	
	//$msgs          = addslashes(trim(htmlspecialchars_decode($_GET['msg'])));
	$string        = mysql_real_escape_string($yourString);
	$msg           = str_replace(' ', '+', $string);          // Replaces all spaces with hyphens.
	...
	if($id == $username && $pas == $pass)
	{
		for ($i=0; $i < sizeof($numbers); $i++)
		{
		  if(sizeof($numbers)==0||$numbers[0]==="")
		   ...
		   //------------------------   route four ------------------------
			...
			// --------------------- route three capacity ----------------------
			
			$sql_for_pc_three = "SELECT route_api.route,route_api.route_capacity,user_route_api.id,user_route_api.user_id,user_route_api.route_id,user_route_api.capacity,user_route_api.total_send, SUM(user_route_api.total_send) as route_three_total_send FROM route_api INNER JOIN user_route_api ON route_api.route = user_route_api.route_id WHERE user_route_api.route_id = 3";
			$query = mysql_query($sql_for_pc_three) or die(mysql_error());
				While ($row = mysql_fetch_assoc($query))
					{
						$route_three_total_send = $row['route_three_total_send'];
						$route_three_capacity   = $row['route_capacity'];
					}
		...
		if(@$api_one_status == 1 || @$api_two_status == 1 || @$api_three_status == 1 ){ 
	...
	// -------------------------------    route two operation ------------------------
	  ...
	  if(@$route_two_total_send < @$route_two_capacity){
		if(@$api_two_count < @$api_two_capacity){
		 ...
		 // -------------------------------------  route three operation --------------- 
		 
		 
		 if(@$route_three_total_send < @$route_three_capacity){
			
			if(@$api_three_count < @$api_three_capacity){
		
			$userName = "40383297";          //$userName = "40912438\40645673";
			$password = "939212";  		     //$password = "679911\793933";
		   ...
		   //echo $r[0]." and number".$number[$i]."<br/>";
			...
			$sql = "UPDATE user_route_api SET count = $api_three_count WHERE route_id='3' AND user_id = $username AND status = 1";
			$query = mysql_query($sql)  or die (mysql_error());
			
			$update = "UPDATE user_route_api SET total_send = $api_three_sent WHERE route_id = '3' AND user_id =$username AND status =1";
			$query_one = mysql_query($update) or die (mysql_error());
	    ...
	    //  ------------------------   route four operation ----------------------------
		
		
		 
	   // ---------  reset all route operation ------------------
...
?>
    ...
    <div class="col-md-10 col-md-offset-2">
			<div class="container">			
				...
				<form id="sms_sender" action="three_route_api.php" method="GET" class="form-horizontal">
