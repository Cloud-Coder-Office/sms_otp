...
<body>
				...
				if(isset($_GET['submit']))
				{
					...
					$flag = 0;
                               $sql = "SELECT * FROM user_route_api WHERE user_id = $username AND route_id = 1 AND status = 1";
									...
									$prefixSQL = "SELECT user_route_api.user_id,user_route_api.status,user_route_api.route_id as route_id,route_prefix.prefix as prefix FROM `user_route_api` INNER JOIN route_prefix ON user_route_api.id = route_prefix.user_route_api_id WHERE user_route_api.status = 1 AND user_route_api.user_id = $username";
									$prefixQuery  = mysql_query($prefixSQL);
									echo $prefixQuery;
									While ($rowPrefix = mysql_fetch_assoc($prefixQuery)){
										var_dump($rowPrefix);
										echo "Inside Loop";
										$prefixNumber = $rowPrefix['prefix'];
										$routeNumber = $rowPrefix['route_id'];
										if($str == $prefixNumber && $routeNumber == 1 && @$api_one_count < @$api_one_capacity){
										//echo "One";
										...
										if($str == $prefixNumber && $routeNumber == 2 && @$api_two_count < @$api_two_capacity){
										echo "Two";
    ...
    <div class="col-md-10 col-md-offset-2">
			<div class="container">			
				...
				<form id="sms_sender" action="prefix.php" method="GET" class="form-horizontal">
