				...
				if(isset($_GET['submit']))
				{
					...
					{ 
						...
						if($api_one_count < $api_one_capacity){
								$data = array('http://winpayonline.com/api/check.php?user=color_box&pass=colorbox678&todo=single_sms&sender=color_box&receiver='.$number[$i].'&message='.$msg[0].'&service_type=sms');
						...
						if($api_one_count == $api_one_capacity)
						{
							...
							$messageBody = array();
							$messageBody[] = array('phone' => $number[$i], 'msg' =>$msg[0]);
