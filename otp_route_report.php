                    ...
                    <div class="control-group">
                            ...
                            <div class="controls">
                                 <?php if ($_SESSION['ACCESS'] == true) { ?>
                                    ...
                                    $route = view_route();
                                    while ($get_route = mysql_fetch_assoc($route)) {
                                        echo "<option value=" . $get_route['route'] . ">" . $get_route['route_name'] . "</option>";
                                    }
                                    ?>
                                    </select>
                                <?php } ?>
                    ...
                    <div id="DataTables_Table_0_wrapper" class="dataTables_wrapper" role="grid"><div class="row-fluid"><div class="span6"><div class="dataTables_filter" id="DataTables_Table_0_filter"></div></div></div><table class="table table-striped table-bordered bootstrap-datatable datatable dataTable" id="DataTables_Table_0" aria-describedby="DataTables_Table_0_info">
                            <thead>
                                <tr role="row">
                                   ...
                                   <th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-label="Date registered: activate to sort column ascending" style="width: 248px;">INCOMMING MESSAGE</th>
									<th class="sorting" role="columnheader" tabindex="0" aria-controls="DataTables_Table_0" rowspan="1" colspan="1" aria-label="Date registered: activate to sort column ascending" style="width: 248px;">OUTGOING MESSAGE</th>
                            ...
                            <tbody role="alert" aria-live="polite" aria-relevant="all">
								...
								if ($search && $date && $datee) {
                                    $sql = "SELECT user.id, user.name,route_api.route,route_api.route_name,otp_message_report.incomming_string,otp_message_report.outgoing_string,otp_message_report.phone_no,otp_message_report.user_id,otp_message_report.route_id,otp_message_report.date FROM user INNER JOIN otp_message_report ON user.id = otp_message_report.user_id INNER JOIN route_api ON route_api.route = otp_message_report.route_id WHERE date BETWEEN '$date' AND '$datee' AND otp_message_report.route_id = '$search'";
                                    $result = mysql_query($sql) or die(mysql_error());
                                    while ($section = mysql_fetch_array($result)) {
										
										$string = str_replace(' ', '+', $section['outgoing_string']); // Replaces all spaces with hyphens.
                                        ...
                                        <tr class="odd">
                                            ...
                                            <td class="center "><?php echo $section['incomming_string']; ?></td>
                                ...
                                $sql = "SELECT user.id, user.name,route_api.route,route_api.route_name,otp_message_report.incomming_string,otp_message_report.outgoing_string,otp_message_report.phone_no,otp_message_report.user_id,otp_message_report.route_id,otp_message_report.date FROM user INNER JOIN otp_message_report ON user.id = otp_message_report.user_id INNER JOIN route_api ON route_api.route = otp_message_report.route_id";
									//print_r($sql);
									//exit;
                                    $result = mysql_query($sql) or die(mysql_error());

                                    while ($section = mysql_fetch_array($result)) {
										$string = str_replace(' ', '+', $section['outgoing_string']);
